...
- **Thư viện bổ sung (v1.x)**: `xlsx` (SheetJS — import/export Excel, cài từ CDN chính chủ), `dompurify` (làm sạch HTML ghi chú trước khi `v-html`), `vite-plugin-pwa` (PWA: manifest + service worker)
...
- Teacher = **chủ** (`meta/config.teacherUid`) HOẶC email (đã xác thực) ∈ `meta/config.adminEmails` (admin phụ). Chỉ chủ sửa được `adminEmails`.
- Bootstrap: lần đầu cho user có email khớp `ngominhthuan.tnut@gmail.com` claim
- Parent: anonymous, get-only theo ID, không list collection
- Collections: `meta/config`, `classes`, `students` (+ `months`), `parentLinks/{token}`, `studentCodes/{code}` (mã tra cứu → token; get công khai, ghi chỉ teacher)
- Khi sửa rules → **deploy lại** (`firebase deploy --only firestore:rules`) thì tính năng mới mới chạy trên production
